-------------------------------------------------------------------------------
-- tb_booth_mult_fsm.vhd
-- Self-checking testbench for booth_mult_fsm.
-- Test vectors (operands + expected product_hi/product_lo/range_ovf) were
-- cross-checked against a Python golden model (2000+ random signed 16x16
-- multiplications, all exact) before being embedded here.
-- Includes a watchdog so a stuck FSM fails the test instead of hanging.
-------------------------------------------------------------------------------
library ieee;
...
entity tb_booth_mult_fsm is
end entity tb_booth_mult_fsm;

architecture sim of tb_booth_mult_fsm is
...
begin
    ...
    dut : entity work.booth_mult_fsm
        port map (
            clk        => clk,
            rst        => rst,
            start      => start,
    ...
    begin
        ...
        wait;  -- stop toggling once stim process signals completion
    end process;
    ...
    begin
        ...
        for i in TESTS'range loop
            ...
            -- Watchdog: FSM must assert done within a bounded number of cycles
            cycles := 0;
            ...
            wait until rising_edge(clk);  -- let FSM return to IDLE
        end loop;

        wait for 20 ns;
        report "=================================================";
        report "booth_mult_fsm TB complete. PASS=" &
