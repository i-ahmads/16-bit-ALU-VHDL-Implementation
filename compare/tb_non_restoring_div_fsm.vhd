-- Self-checking testbench for non_restoring_div_fsm.
-- IDENTICAL TESTS array to tb_restoring_div_fsm.vhd (same dividend/divisor
-- pairs, same expected quotient/remainder, same divide-by-zero case) so the
-- two divider architectures are exercised with exactly the same
-- input/output pairs, as requested in course feedback. The only addition
-- versus tb_restoring_div_fsm is a per-vector clock-cycle counter, printed
-- for every test, used to show non-restoring division's data-dependent
-- correction-cycle behaviour (17 cycles normally, 18 whenever the final
-- remainder needs restoring) in the comparative latency table.
library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;
use work.alu_pkg.all;

entity tb_non_restoring_div_fsm is
end entity tb_non_restoring_div_fsm;

architecture sim of tb_non_restoring_div_fsm is
    ...
    -- Identical to tb_restoring_div_fsm.vhd's TESTS array.
...
begin
    ...
    dut : entity work.non_restoring_div_fsm
        port map (
            clk         => clk,
            rst         => rst,
            start       => start,
    ...
    begin
        ...
        wait for 20 ns;
        report "=================================================";
        report "non_restoring_div_fsm TB complete. PASS=" &
